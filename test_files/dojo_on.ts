...
import dojoDebounce = require("dojo/on/debounce");
import dojoThrottle = require("dojo/on/throttle");
  
	doh.register("tests.on.delegate", [
		function matches(){
			var eDiv = <HTMLDivElement> document.body.appendChild(document.createElement("div")),
				eDiv2 = <HTMLDivElement> eDiv.appendChild(document.createElement("div")),
				eSpan = <HTMLSpanElement> eDiv2.appendChild(document.createElement("span")),
				matchResult = [],
				handle = on(eDiv, "click", function(e: Event){
					matchResult.push(!!on.matches(<HTMLDivElement> e.target, 'span:click', this));
					matchResult.push(!!on.matches(<HTMLDivElement> e.target, 'div:click', this));
					matchResult.push(!!on.matches(<HTMLDivElement> e.target, 'div:click', this, false));
					matchResult.push(!!on.matches(<HTMLDivElement> e.target, 'body:click', this));
				});
		...
		function debounce(){
			var eDiv = document.body.appendChild(document.createElement("div")),
				...
				eButton = <HTMLButtonElement> eA.appendChild(document.createElement("button")),
				debouncedCount = 0,
				debouncedCount2 = 0,
				debouncedCount3 = 0,
	...
	doh.register("tests.on", [
		...
		function dom(t){
			...
			var button = <HTMLButtonElement> span.appendChild(document.createElement("button")),
			...
			var eventEmitted;
			var iframe = <HTMLIFrameElement> domConstruct.place('<iframe></iframe>', document.body);
			...
			if(query){
				...
				var textnodespan = <HTMLSpanElement> div.appendChild(document.createElement("span"));
		...
		function extensionEvent(t){
			var div = document.body.appendChild(document.createElement("div"));
			var span = <HTMLSpanElement> div.appendChild(document.createElement("span"));
    ...
    // 
		...
		function pubsub(t){
			...
			topic.publish("/test/foo", {foo: "value"});
			t.is(1, fooCount);
		},
		function touch(t){
			console.log("has", has);
		...
		function stopImmediatePropagation(t){
			var button = <HTMLButtonElement> document.body.appendChild(document.createElement("button"));
		...
		function eventAugmentation(t){
			var div = <HTMLDivElement> document.body.appendChild(document.createElement("div"));
			var button = <HTMLButtonElement> div.appendChild(document.createElement("button"));
			on(button, "click", function(event){
				event.modified = true;
				event.test = 3;
			});
		...
		function delegatePreventDefault(t){
			...
			var cb = <HTMLInputElement> div.childNodes[0];
